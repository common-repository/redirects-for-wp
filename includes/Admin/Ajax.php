...
class Ajax {
    public function __construct()
    {
        ...
        add_action('wp_ajax_wp301redirects/admin/create_new_link', [$this, 'create_new_link']);
        add_action('wp_ajax_wp301redirects/admin/update_link', [$this, 'update_link']);
        add_action('wp_ajax_wp301redirects/admin/delete_link', [$this, 'delete_link']);
    }
    public function get_wildcard()
    {
        check_ajax_referer('wp301redirects', 'security');
        if( ! current_user_can( 'manage_options' ) ) wp_die();
		wp_send_json_success(get_option('wp_301_redirects_wildcard'));
		wp_die();
    }
    public function wildcard() 
    {
        check_ajax_referer('wp301redirects', 'security');
        if( ! current_user_can( 'manage_options' ) ) wp_die();
        update_option('wp_301_redirects_wildcard', sanitize_text_field($_POST['toggle']));
    ...
    public function activate_plugin()
    {
        ...
        if ($result === false) {
            wp_send_json_error(__('Plugin couldn\'t be activated.', 'wp-301-redirects'));
        }
        wp_send_json_success(__('BetterLinks is activated!', 'wp-301-redirects'));
        wp_die();
    }
    public function hide_notice()
    {
        ...
        update_option('wp301redirects_hide_btl_notice', $hide);
    ...
    public function fetch_all_links()
    {
        check_ajax_referer('wp301redirects', 'security');
        if( ! current_user_can( 'manage_options' ) ) wp_die();
        wp_send_json_success(get_option('wp_301_redirects'));
        wp_die();
    }
    ...
    public function create_new_link()
    {
        ...
        $links = get_option('wp_301_redirects');
		if(!empty($key) && !isset($links[$key])){
			$links[$key] = $value;
			update_option('wp_301_redirects', $links);
		}
